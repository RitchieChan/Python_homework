...
class Draw_map():
    ...
    '''
    def to_map_city(self,area, variate,province,update_time,curConfirm):
        pieces = [
        ...
        c = (
            ...
            #.add("现有确诊",[list(z) for z in zip(area, curConfirm)],province, is_map_symbol_show=False)
    ...
    def to_map_china(self,area, variate,update_time,curConfirm):
        ...
        c = (
            ...
            Map(init_opts=opts.InitOpts(width='1000px', height='880px'))
                ...
                .add("现有确诊",[list(z) for z in zip(area, curConfirm)],"china", is_map_symbol_show=False)
                .set_global_opts(
    ...
    def to_map_china_travel(self,area, travel_factor,update_time):
        
       
        pieces = [{"max": 999999, "min": 1001, "label": ">10000", "color": "#FFFFFF"},
                  {"max": 9999, "min": 1000, "label": "1000-9999", "color": "#F5A9A9"},
                  {"max": 999, "min": 100, "label": "100-999", "color": "#F78181"},
                  {"max": 99, "min": 10, "label": "10-99", "color": "#DF0101"},
                  {"max": 9, "min": 1, "label": "1-9", "color": "#B40404"},
                  {"max": 0, "min": 0, "label": "0", "color": "#8A0808"},
                  ]

        c = (
            # 设置地图大小
            Map(init_opts=opts.InitOpts(width='1000px', height='880px'))
                .add("旅游负因子",[list(z) for z in zip(area, travel_factor)],"china", is_map_symbol_show=False)
                .set_global_opts(
                title_opts=opts.TitleOpts(title="中国暑假疫情旅游负因子地图分布", subtitle='截止%s 中国暑假疫情旅游负因子分布情况'%(update_time), pos_left="center", pos_top="10px"),
                legend_opts=opts.LegendOpts(is_show=False),
                visualmap_opts=opts.VisualMapOpts(max_=200, is_piecewise=True,
                                                  pieces=pieces,
                                                  ),
                ...
                .render("./map/中国暑假疫情旅游负因子分布图.html")
        )